...
export const fetchWinnersByYearSearchRepository = async (
    params: IFetchMoviesParams,
    ...
    try {
        ...
        const response = await axios.get<IMovieApiResponse>(
        ...
        if (axios.isAxiosError(error))
            throw new Error(
                `Failed to fetch movie data: ${error.response?.status} ${error.response?.statusText}`,
            );
        else
            throw new Error(
                'An unexpected error occurred while fetching movie data',
            );
    }
};
