...
export const fetchYearsWithMultipleWinners =
    async (): Promise<YearApiResponse> => {
        try {
            const response = await axios.get<YearApiResponse>(
            ...
            if (axios.isAxiosError(error))
                throw new Error(
                    `Failed to fetch years data: ${error.response?.status} ${error.response?.statusText}`,
                );
            else
                throw new Error(
                    'An unexpected error occurred while fetching years data',
                );
        }
    };
